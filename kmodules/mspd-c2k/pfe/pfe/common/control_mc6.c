...
static void hw_MC6_entry_delayed_remove(void)
{
	...
	dlist_for_each_safe(hw_entry, entry, &hw_mc6_removal_list, list)
	{
		...
		dma_pool_free(ctrl->dma_pool, hw_entry, be32_to_cpu(hw_entry->dma_addr));
	}
}
...
{
	return kzalloc(sizeof(struct _tMC6Entry), GFP_KERNEL);
}
...
static int MC6_entry_add(PMC6Entry this_entry, U32 hash)
{
	...
	hw_entry = dma_pool_alloc(ctrl->dma_pool, GFP_ATOMIC, &dma_addr);
	if (!hw_entry)
	{
...
/* add a hardware entry to packet engine hash */
static void MC6_entry_link(struct _thw_MC6Entry *hw_entry, U32 hash)
{
    struct _thw_MC6Entry *hw_entry_first;
...
/* remove a hardware entry from the packet engine hash */
static void MC6_entry_unlink(struct _thw_MC6Entry *hw_entry, U32 hash)
{
...
#ifdef WIFI_ENABLE
			if (this_entry->mcdest.num_listeners == 0)
#else
...
static int MC6_is_listener(PMC6Entry this_group, POnifDesc onif_desc)
{
	int i;
...
static int MC6_command_add(PMC6Command cmd)
{
	...
	hash_key = HASH_MC6(cmd->dst_addr[IP6_LO_ADDR]);
	...
	/* Check if overlapping pair is already present.
	...
	if(entry == NULL)
	{
		...
		if(rtp_mc6_link_stats_entry_by_tuple((void *)this_entry, this_entry->src_addr, this_entry->dst_addr) == ERR_RTP_STATS_DUPLICATED)
			this_entry->status |= CONNTRACK_RTP_STATS;
#endif
	}
	...
	for (i = 0; i < cmd->num_output; i++)
	{
		...
		if (onif_desc)
		{
			if (!(onif_desc->itf->type & IF_TYPE_TUNNEL) && (MC6_is_listener(this_entry, onif_desc) < 0))  // ignore if listener already exists or is a tunnel interface
			{
...
found:
	...
	for(i = 0; i <= cmd->num_output; i++)
	{
		...
		if(onif_desc)
		{
			if((listener_index = MC6_is_listener(this_entry, onif_desc)) >= 0)
			{
				/* remove listener, and move following entries up */
...
static int MC6_command_refresh(PMC6Command cmd)
{
	...
	if(this_entry != NULL)
	{
		...
		for(listener_refreshed = 0; listener_refreshed < cmd->num_output; listener_refreshed++)
		{
			...
			{
				if ((listener_index = MC6_is_listener(this_entry, onif_desc)) >= 0) {
					this_entry->mcdest.listeners[listener_index].timer = cmd->output_list[listener_refreshed].timer;
					this_entry->mcdest.listeners[listener_index].shaper_mask = cmd->output_list[listener_refreshed].shaper_mask;
				}	
			}
			else
			{
			...
			if (cmd->output_list[listener_refreshed].timer > 0)
