...
extern void free_packet(struct tMetadata *);
...
static void data_channel_put_event(U32 eventnum, struct tMetadata *pMetadata, BOOL do_set_event)
{
...
extern GETFILLHANDLER gGetFillTable[EVENT_MAX];
...
static __inline void data_channel_put_event(U32 eventnum, struct tMetadata *pMetadata, BOOL do_set_event)
{
        ...
        PDataChannel pChannel = &gCH_table[eventnum];
        pktqueue_put(pChannel, pMetadata); 
                //set_event(gEventStatusReg, 1 << eventnum);
}
...
#define	SEND_TO_CHANNEL(eventnum, pmtd)	data_channel_put_event(eventnum, pmtd, FALSE)
