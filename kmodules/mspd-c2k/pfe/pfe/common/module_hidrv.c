...
HostMessage *msg_alloc(void)
{
	/* FIXME allocate BMU1 buffer */
	return NULL;
}

void msg_free(HostMessage *msg)
{
	/* FIXME free BMU1 buffer, probably never called since it's the hardware that frees the message buffer */
}

void msg_send(HostMessage *msg)
{
	/* FIXME send message to host through HIF */
}
