...
//		break;
	...
	pTunnelEntry->priority = (pTunnelEntry->fl & 0xf0) >> 5;
...
void tnl_update(int tunnel_index)
{
	...
	else
	{
		...
		if(pTunnelEntry->pRtEntry)
		{
			phw_tnl->route.itf = cpu_to_be32(virt_to_class_dmem(pTunnelEntry->pRtEntry->itf));
			...
			phw_tnl->route.dAddr = pTunnelEntry->pRtEntry->dAddr;
		}
		else
		{
		...
		phw_tnl->fl = cpu_to_be32(pTunnelEntry->fl);
		phw_tnl->frag_off = cpu_to_be16(pTunnelEntry->frag_off);
