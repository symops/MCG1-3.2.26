...
PCtEntry ct_alloc(void)
{
	...
	pEntry_orig = kzalloc(2 * sizeof(CtEntry), GFP_KERNEL);
	if (!pEntry_orig)
		return NULL;
...
/** Adds a software conntrack to the hardware hash (making it visible to PFE).
* If the conntrack is enabled and the route entry is available,
* a new hardware conntrack added to the hardware hash.
* In case of error the conntrack is not added to the hash.
*
...
static int hw_ct_add_one(PCtEntry pEntry, U32 hash)
{
	...
	/* If disabled don't configure pfe */
	if (pEntry->status & CONNTRACK_FF_DISABLED)
		goto out;
	
	if (IS_NULL_ROUTE(pEntry->pRtEntry))
	{
		/* Attach to route */
		if (!IP_Check_Route(pEntry))
		{
...
//	printk(KERN_INFO "%s %p\n", __func__, ct);
	...
	ct->inPhyPortNum = pEntry->inPhyPortNum;
	...
	ct->route.itf = cpu_to_be32(virt_to_class_dmem(pEntry->pRtEntry->itf));
...
//	ct->route.flags = pEntry->pRtEntry->flags;
	ct->route.dAddr = pEntry->pRtEntry->dAddr;
...
* The hardware conntrack be can be added (if none existed), removed (if it's now disabled) or update.
*
* @param pEntry		pointer to the software conntrack
...
* @return		NO_ERR in case of success, ERR_xxx in case of error
*/
int ct_update_one(PCtEntry pEntry, U32 hash)
{
	int rc = NO_ERR;
	int i;
	...
	if (!pEntry->ct)
	{
		rc = hw_ct_add_one(pEntry, hash);
	}
	else
	{
		if (pEntry->status & CONNTRACK_FF_DISABLED)
		{
			hw_ct_remove_one(pEntry, hash);
			goto out;
		}
		...
		hw_ct_set_flags(pEntry->ct, CT_USED | CT_UPDATING | CT_VALID);

		/* Update hw entry */
		/* 5-tuple can not change, no need to update all fields */
		pEntry->ct->fwmark = cpu_to_be16(pEntry->fwmark);
		pEntry->ct->status = cpu_to_be16(pEntry->status);

		pEntry->ct->route.itf = cpu_to_be32(virt_to_class_dmem(pEntry->pRtEntry->itf));
		memcpy(pEntry->ct->route.dstmac, pEntry->pRtEntry->dstmac, ETHER_ADDR_LEN);
		pEntry->ct->route.mtu = cpu_to_be16(pEntry->pRtEntry->mtu);
		//pEntry->ct->route.flags = pEntry->pRtEntry->flags;
		pEntry->ct->route.dAddr = pEntry->pRtEntry->dAddr;
		
		for (i = 0; i < SA_MAX_OP; i++)
		{
			pEntry->ct->hSAEntry[i] = cpu_to_be16(pEntry->hSAEntry[i]);
		}

		hw_ct_set_flags(pEntry->ct, CT_USED | CT_VALID);
	}
...
int ct_update(PCtEntry pEntry_orig, PCtEntry pEntry_rep, U32 hash_orig, U32 hash_rep)
{
	...
	rc1 = ct_update_one(pEntry_orig, hash_orig);
	rc2 = ct_update_one(pEntry_rep, hash_rep);
...
PRouteEntry IP_Check_Route(PCtEntry pCtEntry)
{
	...
	// direction is upstream (output_port = 0).  If this is not true, then we must swap
	// the queue and DSCP marking parameters with the twin connection.

	if (IS_IPV4(pCtEntry))
		pCtTwin = CT_TWIN(pCtEntry);
	else
		pCtTwin = (PCtEntry)CT6_TWIN(pCtEntry);

	if (IS_NULL_ROUTE(pCtTwin->pRtEntry)) {
		// if CtTwin route is non null, swap has already been performed
		if ( ((CT_REPLY_BIT(pCtEntry) == 0) && itf_get_phys_port(pRtEntry->itf) == 1) ||		/* FIXME does this work for wifi offload ? */
			(CT_REPLY_BIT(pCtEntry) && itf_get_phys_port(pRtEntry->itf) == 0) )
		{
			pCtEntry->status |= CONNTRACK_MARKSWAP;
			pCtTwin->status |= CONNTRACK_MARKSWAP;
			IP_MarkSwap(pCtEntry, pCtTwin);
		}
	}
...
void IP_MarkSwap(PCtEntry pCtEntry, PCtEntry pCtTwin)
{
	...
	queue = pCtEntry->queue;
	pCtEntry->queue = pCtTwin->queue;
	pCtTwin->queue = queue;

	dscp_mark_flag = pCtEntry->dscp_mark_flag;
	pCtEntry->dscp_mark_flag = pCtTwin->dscp_mark_flag;
	pCtTwin->dscp_mark_flag = dscp_mark_flag;

	dscp_mark_value = pCtEntry->dscp_mark_value;
	pCtEntry->dscp_mark_value = pCtTwin->dscp_mark_value;
	pCtTwin->dscp_mark_value = dscp_mark_value;
}
...
{
	U32 fwmark;
	if (pOrigEntry->status & CONNTRACK_MARKSWAP)
	{
		PCtEntry temp;
		temp = pOrigEntry;
		pOrigEntry = pReplEntry;
		pReplEntry = temp;
	}
...
U32 Get_Ctentry_Hash(PVOID pblock)
{
	...
	U32 hash;
	U32 protocol;
	protocol = (pctentry->status & CONNTRACK_UDP) ? IPPROTOCOL_UDP : IPPROTOCOL_TCP;
	if (IS_IPV4(pblock))
	{
...
int IPv4_delete_CTpair(PCtEntry ctEntry)
{
	...
	message->fwmark = IP_get_fwmark(ctEntry, twin_entry);
...
restart_loop:
		slist_for_each(pCtEntry, entry, &ct_cache[i], list)
		{
			if ((pCtEntry->status & CONNTRACK_ORIG) == CONNTRACK_ORIG)  // only check orig ctEntries (not their twins)
			{
				...
				if (timer > GET_TIMEOUT_VALUE(pCtEntry->status & CONNTRACK_UDP, bidir_flag)
						...
						if (newblock != NULL)
						{
							...
							if(pCtEntry->status & CONNTRACK_RTP_STATS) {
								...
								if (IS_IPV4(pCtEntry)) {
									...
									rtp_ipv4_link_stats_entry_by_tuple((void *)pCtEntry, pCtEntry->Saddr_v4, pCtEntry->Daddr_v4, pCtEntry->Sport, pCtEntry->Dport);
								} else {
									pCT6Entry = (PCtEntryIPv6)pCtEntry;
									rtp_ipv6_link_stats_entry_by_tuple((void *)pCT6Entry, pCT6Entry->Saddr_v6, pCT6Entry->Daddr_v6, pCT6Entry->Sport, pCT6Entry->Dport);
								}
							}
							...
							if(twin_entry->status & CONNTRACK_RTP_STATS) {
								...
								if (IS_IPV4(twin_entry)) {
									/* check if rtp stats entry is created for this conntrack, if found link the two object */
									rtp_ipv4_link_stats_entry_by_tuple((void *)twin_entry, twin_entry->Saddr_v4, twin_entry->Daddr_v4, twin_entry->Sport, twin_entry->Dport);
								} else {
									twin6_entry = (PCtEntryIPv6)twin_entry;
									rtp_ipv6_link_stats_entry_by_tuple((void *)twin6_entry, twin6_entry->Saddr_v6, twin6_entry->Daddr_v6, twin6_entry->Sport, twin6_entry->Dport);
								}
							}
						}
					}
	...
	switch(Ctcmd.action)
	{
		...
		case ACTION_REGISTER: 
	  			...
	  			if(Ctcmd.protocol == IPPROTOCOL_UDP)
				{
					...
					if(rtp_ipv4_link_stats_entry_by_tuple((void *)pEntry_orig, pEntry_orig->Saddr_v4, pEntry_orig->Daddr_v4, pEntry_orig->Sport, pEntry_orig->Dport) == ERR_RTP_STATS_DUPLICATED)
						pEntry_orig->status |= CONNTRACK_RTP_STATS;
					if(rtp_ipv4_link_stats_entry_by_tuple((void *)pEntry_rep, pEntry_rep->Saddr_v4, pEntry_rep->Daddr_v4, pEntry_rep->Sport, pEntry_rep->Dport) == ERR_RTP_STATS_DUPLICATED)
		...
		case ACTION_UPDATE: 
				...
				if (pEntry_orig->status & CONNTRACK_MARKSWAP)
					IP_MarkSwap(pEntry_orig, pEntry_rep);
				...
				return ct_update(pEntry_orig, pEntry_rep, hash_key_ct_orig, hash_key_ct_rep);
...
static int IPv4_HandleIP_Set_FragTimeout(U16 *p, U16 Length)
{
...
#if defined(COMCERTO_2000)
	ipv4_frag_timeout = cpu_to_be16(ipv4_frag_timeout);
	ipv4_frag_expirydrop = cpu_to_be16(ipv4_frag_expirydrop);
	/*Write two U16 in single go*/
	pe_dmem_memcpy_to32(UTIL_ID, virt_to_util_dmem(&(UTIL_VARNAME2(ipv4_frag_timeout))), &ipv4_frag_timeout, sizeof(int));
#endif
...
static U16 M_ipv4_cmdproc(U16 cmd_code, U16 cmd_len, U16 *pcmd)
{
	...
	switch (cmd_code)
	{
		...
		case CMD_IPV4_FRAGTIMEOUT:
			rc = IPv4_HandleIP_Set_FragTimeout(pcmd, cmd_len);
			break;
...
//		hw_ct_set_next(ct, ctrl->null_ct_dma_handle); /* only needed if doing hardware fetch */
		hw_ct_set_next(ct, 0);
		hw_ct_set_flags(ct, 0);
	}
#endif
	...
	udp_unidir_timeout = UDP_UNIDIR_TIMEOUT * CT_TICKS_PER_SECOND;
	udp_bidir_timeout = UDP_BIDIR_TIMEOUT * CT_TICKS_PER_SECOND;
	tcp_timeout = TCP_TIMEOUT * CT_TICKS_PER_SECOND;
