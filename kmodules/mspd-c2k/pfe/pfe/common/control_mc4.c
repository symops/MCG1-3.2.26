...
{
	return kzalloc(sizeof(struct _tMC4Entry), GFP_KERNEL);
}
...
static void MC4_entry_link(struct _thw_MC4Entry *hw_entry, U32 hash)
{
    struct _thw_MC4Entry *hw_entry_first;
...
static void MC4_entry_unlink(struct _thw_MC4Entry *hw_entry, U32 hash)
{
...
static int MC4_is_listener(PMC4Entry this_group, POnifDesc onif_desc)
{
	int i;
...
static int MC4_command_add(PMC4Command cmd)
{
	...
	if(entry == NULL)
	{
		...
		if(rtp_mc4_link_stats_entry_by_tuple((void *)this_entry, this_entry->src_addr, this_entry->dst_addr) == ERR_RTP_STATS_DUPLICATED)
	...
	for (i = 0; i < cmd->num_output; i++)
	{
		...
		if (onif_desc)
		{
			if (!(onif_desc->itf->type & IF_TYPE_TUNNEL) && (MC4_is_listener(this_entry, onif_desc) < 0))  // ignore if listener already exists or is a tunnel interface
			{
...
found:
	...
	for(i = 0; i <= cmd->num_output; i++)
	{
		...
		if(onif_desc)
		{
			if((listener_index = MC4_is_listener(this_entry, onif_desc)) >= 0)
			{
				/* remove listener, and move following entries up */
