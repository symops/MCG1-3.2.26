...
extern struct _tbridge_enable L2bridge_enabled[];
#else
...
PL2Flow_entry M_bridge_find_l2_flow_entry(Metadata * mtd, U32 hash, struct L2Flow_tmp * l2flow);
PL3Flow_entry M_bridge_find_l3_flow_entry(Metadata * mtd, U32 hash, struct L3Flow * l3flow);
...
static __inline U32 HASH_L2FLOW(struct L2Flow_tmp *l2flow)
{
	...
	sum = READ_L2_FIELD_INT(&lp->x[0]) + READ_L2_FIELD_INT(&lp->x[1]) + READ_L2_FIELD_INT(&lp->x[2]) + l2flow->vlan_tag;
	sum += (sum >> 16) + l2flow->ethertype +  l2flow->session_id;
	hash = (sum >> 8) ^ (sum & 0xFF);
...
{
	U32 sum;
	sum = l3flow->saddr[0] + ((l3flow->daddr[0] << 7) | (l3flow->daddr[0] >> 25));
	sum ^= l3flow->saddr[IP6_LO_ADDR] + ((l3flow->daddr[IP6_LO_ADDR] << 7) | (l3flow->daddr[IP6_LO_ADDR] >> 25));
	sum ^= l3flow->sport + ((l3flow->dport << 11) | (l3flow->dport >> 21));
	sum ^= (sum >> 16);
	sum ^= (sum >> 8);
...
/* PBUF route entry memory chunk (128B) is used to allocated a new L2 flow entry */
#define bridge_l2_get_buffer(buf_i)\
	((PVOID)(CLASS_ROUTE0_BASE_ADDR + buf_i * CLASS_ROUTE_SIZE))

/* second half of the route entry memory chunk is use to allocated the corresponding L3 flow entry
This means sum of L2 and L3 flow entries must not exceed 128 bytes each. Maybe add
a compilation time check about this requirement */	
#define bridge_l3_get_buffer(buf_i)\
	(bridge_l2_get_buffer(buf_i) + sizeof(L2Bridge_entry))

//FIXME!!  The above is not true -- L3Flow entry is larger than 64 bytes.
