...
struct class_stats CLASS_DMEM_SH2(stats);
...
static void stats_interface_pkt_reset(U8 interface)
{
	...
	STATISTICS_CLASS_GET(total_bytes_received[interface], temp, TRUE);
	STATISTICS_CLASS_GET(total_pkts_received[interface], temp, TRUE);
...
{
	STATISTICS_CLASS_GET(total_bytes_received[interface], rsp->total_bytes_received[0], do_reset);
	STATISTICS_CLASS_GET(total_pkts_received[interface], rsp->total_pkts_received, do_reset);
...
int pe_send_stats_cmd(u16 cmd_type, u16 action)
{
	struct pfe_ctrl *ctrl = &pfe->ctrl;
	if (pe_request(ctrl,UTIL_ID, cmd_type, action, 0,0) < 0)
                return CMD_ERR ;
	return 0;
}
...
static U16 M_stat_cmdproc(U16 cmd_code, U16 cmd_len, U16 *pcmd)
{
		...
		if(gFpCtrlStatFeatureBitMask & STAT_BRIDGE_BITMASK) {
			...
			if(action == FPP_STAT_RESET)
			{
				...
				for(x=0; x<NUM_BT_ENTRIES;x++) {
					pEntry = container_of(&bridge_cache[x], typeof(L2Bridge_entry), list);
	...
	case CMD_STAT_IPSEC_STATUS:	{
		...
		if(gFpCtrlStatFeatureBitMask & STAT_IPSEC_BITMASK) {
			...
			{
#if defined(COMCERTO_2000)
				pe_send_stats_cmd(CMD_STAT_IPSEC_STATUS, action);
#endif
...
#else
	gFpCtrlStatistics.FS_max_active_connections = 16*K; /* FIXME */
#endif
	gStatBridgeQueryStatus = 0;
	gStatPPPoEQueryStatus = 0;
	gStatIpsecQueryStatus = 0;
