...
static int rx_Bridge_Get_Hash_Snapshot(int bt_hash_index, int br_tot_entries, PL2BridgeQueryEntryResponse pBridgeSnapshot)
{
	...
	slist_for_each(pBridgeEntry, entry, &bridge_cache[bt_hash_index], list) 
	{
		if(br_tot_entries > 0)
		{
			...
			if(pBridgeEntry->input_interface > 1)
				SFL_memcpy(pBridgeSnapshot->input_name, get_onif_name(pBridgeEntry->input_interface), 16);
			if(pBridgeEntry->output_itf->index > 1)
...
static int stat_Bridge_Get_Hash_Snapshot(int stat_bt_hash_index, int stat_tot_br_entries, PStatBridgeEntryResponse pStatBridgeSnapshot)
{
	...
	slist_for_each(pStatBridgeEntry, entry, &bridge_cache[stat_bt_hash_index], list)
	{
		if(stat_tot_br_entries > 0)
		{
			...
			if(pStatBridgeEntry->input_interface > 1)
				SFL_memcpy(pStatBridgeSnapshot->input_name, get_onif_name(pStatBridgeEntry->input_interface), 16);
			if(pStatBridgeEntry->output_itf->index > 1)
...
static int rx_Get_Hash_Snapshot_L2FlowEntries(int L2Flow_hash_index, int L2Flow_tot_entries, PL2BridgeL2FlowEntryCommand pL2FlowSnapshot)
{
	...
	if(L2Flow_hash_index < NUM_BT_ENTRIES){
		pL2FlowEntry = container_of(&bridge_cache[L2Flow_hash_index], typeof(L2Flow_entry), list);
		pL3FlowEntry = NULL;
	}
	else{
		pL3FlowEntry = container_of(&bridge_l3_cache[L2Flow_hash_index - NUM_BT_ENTRIES], typeof(L3Flow_entry), list);
	...
	while ((pL2FlowEntry) && (L2Flow_tot_entries > 0)) {
		...
		SFL_memcpy(pL2FlowSnapshot->input_name, get_onif_name(pL2FlowEntry->input_if), INTERFACE_NAME_LENGTH);
		SFL_memcpy(pL2FlowSnapshot->output_name, get_onif_name(pL2FlowEntry->output_if), INTERFACE_NAME_LENGTH);
