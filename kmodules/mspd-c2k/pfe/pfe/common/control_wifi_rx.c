...
static int wifi_vap_entry( U16 *ptr, U16 len )
{
	...
	printk("%s:%d Remove entry\n", __func__, __LINE__);
			...
			remove_onif_by_index(phy_port[portid].itf.index);
	...
	printk("%s:%d\n", __func__, __LINE__);
			if(!add_onif(cmd.ifname, &phy_port[portid].itf, NULL, IF_TYPE_WLAN | IF_TYPE_PHYSICAL))
			{
				return CMD_ERR;
			}
			...
			SFL_memcpy(phy_port[portid].mac_addr, cmd.mac_addr, 6);
#if defined(COMCERTO_2000_CONTROL)
			printk("%s: PHYID:%d vapid:%d mac_addr: %02x:%02x:%02x:%02x:%02x:%02x\n", __func__, portid, cmd.VAPID,
									phy_port[portid].mac_addr[0],
									phy_port[portid].mac_addr[1],
									phy_port[portid].mac_addr[2],
									phy_port[portid].mac_addr[3],
									phy_port[portid].mac_addr[4],
									phy_port[portid].mac_addr[5]);
...
static U16 M_wifi_rx_cmdproc(U16 cmd_code, U16 cmd_len, U16 *pcmd)
{
	...
	printk("%s:%d\n", __func__, __LINE__);
						 ...
						 for (i = 0; i < MAX_WIFI_VAPS; i++)
						 {
							 ...
							 SFL_memcpy(vaps[i].ifname, get_onif_name(phy_port[PORT_WIFI_IDX + i].itf.index), 12);
						 }
	...
	printk("%s:%d\n", __func__, __LINE__);
					 for (i = 0; i < MAX_WIFI_VAPS; i++)
					 {
						 if( wifiDesc[i].VAPID != 0XFFFF )
						 {
							 ...
							 remove_onif_by_index(phy_port[PORT_WIFI_IDX + i].itf.index);
...
#endif
		phy_port[PORT_WIFI_IDX + i].id = PORT_WIFI_IDX + i;
	}
