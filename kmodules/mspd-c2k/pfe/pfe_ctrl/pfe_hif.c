...
//			printk(KERN_INFO "continuation: id %d, q %d, len %d, flags %x\n", hif->client_id, hif->qno, len, local_desc.ctrl);
		...
		if ((hif->client_id >= HIF_CLIENTS_MAX) || !(hif->shm->gClient_status & (1 << hif->client_id))) {
...
#ifdef INT_COAL
	/* Setup Interrupt coalescing to 500us */
	/* Counter freq 250 MHz => 125000 cycles => 0x1e848 */
	writel(0x8001e848, HIF_INT_COAL);
#endif
...
void pfe_hif_exit(struct pfe *pfe)
{
	...
	printk(KERN_INFO "%s\n", __func__);
	hif->shm->gClient_status = 0; /* Make sure all clients are disabled */
