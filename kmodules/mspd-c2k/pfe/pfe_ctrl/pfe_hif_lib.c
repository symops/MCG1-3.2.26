...
unsigned int alloc_on_init = 0;
module_param(alloc_on_init, uint, S_IRUGO);
MODULE_PARM_DESC(alloc_on_init,
                 "0: disabled, allocate rx & tx queues when nic is activated (default); 1: enabled, allocate rx & tx queues when nic is initialized");

unsigned int disable_wifi_offload = 0;
module_param(disable_wifi_offload, uint, S_IRUGO);
MODULE_PARM_DESC(disable_wifi_offload,
                 "0: enable wifi offload (default); 1: disable wifi offload");
...
static int hif_lib_client_init_rx_buffers(struct hif_client_s *client, int q_size)
{
    ...
    if (alloc_on_init) {
        ...
        memset(client->rx_qbase, 0, (client->rx_qn * q_size * sizeof(struct rx_queue_desc)));
    }
    else {
        printk(KERN_INFO "%s: allocating rx queue \n", __func__);
...
static int hif_lib_client_init_tx_buffers(struct hif_client_s *client, int q_size)
{
    ...
    if (alloc_on_init) {
        ...
        memset(client->tx_qbase, 0, (client->tx_qn * q_size * sizeof(struct tx_queue_desc)));
    }
    else {
        printk(KERN_INFO "%s: allocating tx queue \n", __func__);
...
//	printk(KERN_DEBUG "%s : seq: %x id: %x desc_n: %d desc_addr: %p tx_hdr_v : %p tx_hdr_p: %x, ip_off: %d tcp_off: %d\n",__func__, tso_hdr->tcp_seq, tso_hdr->ip_id, pfe->hif.Txtosend, desc, tso_hdr, tso_hdr_p, tso_hdr->ip_off, tso_hdr->tcp_off );
	...
	__hif_xmit_pkt(&pfe->hif, client->id, qno, (void *)tso_hdr_p, sizeof(struct hif_tso_hdr), HIF_DONT_DMA_MAP);
...
static void hif_lib_tmu_credit_init(struct pfe *pfe)
{
	...
	for (i = 0; i < NUM_GEMAC_SUPPORT; i++)
		for (q = 0; q < EMAC_TXQ_CNT; q++) {
			pfe->tmu_credit.tx_credit_max[i][q] = (q < 8) ? 511 : 255;
