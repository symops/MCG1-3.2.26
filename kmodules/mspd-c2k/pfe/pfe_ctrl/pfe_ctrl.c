...
int comcerto_fpp_send_command(u16 fcode, u16 length, u16 *payload, u16 *resp_length, u16 *resp_payload)
{
	struct pfe_ctrl *ctrl = &pfe->ctrl;

	mutex_lock(&ctrl->mutex);

	__pfe_ctrl_cmd_handler(fcode, length, payload, resp_length, resp_payload);
 ...
 *
 */
int comcerto_fpp_register_event_cb(void (*event_cb)(u16, u16, u16*))
{
	struct pfe_ctrl *ctrl = &pfe->ctrl;
...
int pfe_ctrl_init(struct pfe *pfe)
{
	...
	ctrl->dma_pool = dma_pool_create("pfe dma pool", pfe->dev, DMA_BUF_SIZE_256, DMA_BUF_MIN_ALIGNMENT, DMA_BUF_BOUNDARY);
	if (!ctrl->dma_pool)
	{
	...
	ctrl->dma_pool_512 = dma_pool_create("pfe dma pool 512B", pfe->dev, DMA_BUF_SIZE_512, DMA_BUF_MIN_ALIGNMENT, DMA_BUF_BOUNDARY);
	...
	/* FIXME with software fetch we don't need this and instead we can use next=0 as a list terminator */
	ctrl->null_ct = dma_pool_alloc(ctrl->dma_pool, GFP_KERNEL, &ctrl->null_ct_dma_handle);
	if (!ctrl->null_ct)
	{
	...
	memset(ctrl->null_ct, 0, CLASS_ROUTE_SIZE);
...
err4:
	dma_pool_free(ctrl->dma_pool, ctrl->null_ct, ctrl->null_ct_dma_handle);
