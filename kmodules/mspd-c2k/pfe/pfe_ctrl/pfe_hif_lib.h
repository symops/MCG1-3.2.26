...
struct hif_client_shm {
	...
	volatile u32 rx_qbase; /*Rx queue base address */
	volatile u32 rx_qsize; /*each Rx queue size, all Rx queues are of same size */
	volatile u32 tx_qbase; /* Tx queue base address */
	volatile u32 tx_qsize; /*each Tx queue size, all Tx queues are of same size */
};
 ...
 */
struct hif_shm {
	volatile u32 rx_buf_pool_cnt; /*Number of rx buffers available*/
	volatile void *rx_buf_pool[HIF_RX_DESC_NT];/*Rx buffers required to initialize HIF rx descriptors */
	volatile u32 gClient_status; /*Global client status */
	volatile u32 hif_qfull; /*TODO Client-id that caused for the TMU3 queue stop */
	volatile u32 hif_qresume; /*TODO */
...
extern unsigned int alloc_on_init;
extern unsigned int disable_wifi_offload;
