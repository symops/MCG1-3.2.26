...
static int pfediag_enable(int pe_index, int ring_size)
{
	...
	fppdiag_driver_data->pfe_rng_baseaddr = kzalloc((sizeof(u32) * ring_size), GFP_KERNEL);
	if(!fppdiag_driver_data->pfe_rng_baseaddr)
		return -1; /* Failure */

	fppdiag_driver_data->virt_rng_baseaddr = kzalloc((sizeof(void *) * ring_size), GFP_KERNEL);
...
err_ring:
	...
	kfree(fppdiag_driver_data->virt_rng_baseaddr);
	fppdiag_driver_data->virt_rng_baseaddr = NULL;
err_virt:
	kfree(fppdiag_driver_data->pfe_rng_baseaddr);
...
int pfe_diags_init(struct pfe *pfe)
{
	...
	fppdiag_drv_data =  kzalloc(NUM_PE_DIAGS*sizeof(struct fppdiag_drv_dat), GFP_KERNEL);
	if(!fppdiag_drv_data)
	{
		ret = -ENOMEM;
		goto err0;
	...
	//fppdiagctl =  kmalloc(NUM_PE_DIAGS*sizeof(fppdiag_ctl_t), GFP_KERNEL);
...
void pfe_diags_exit(struct pfe *pfe)
{
	...
	kfree(pfe->diags.fppdiag_drv_data);
}
