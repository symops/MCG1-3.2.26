...
{
	if (p)
		return ( (unsigned long) p - (unsigned long) pfe->iram_baseaddr + (unsigned long) pfe->iram_phys_baseaddr );
	else
		return 0;
}
...
unsigned long virt_to_phys_ipsec_lmem(void *p)
{
	struct pfe_ctrl *ctrl = &pfe->ctrl;
        if (p)
                return ( (unsigned long) p - (unsigned long) ctrl->ipsec_lmem_baseaddr + (unsigned long) ctrl->ipsec_lmem_phys_baseaddr );
        else
                return 0;
}
...
{
        if (p)
                return ( (unsigned long) p - (unsigned long) pfe->ipsec_baseaddr + (unsigned long) pfe->ipsec_phys_baseaddr );
        else
                return 0;
}
...
void msg_send(HostMessage *msg)
{
	...
	if (ctrl->event_cb)
		ctrl->event_cb(msg->code, msg->length, msg->data);
