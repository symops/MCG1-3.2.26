...
static ssize_t pe_status(char *buf, int id, u32 dmem_addr, unsigned long do_clear)
{
	...
	*(u32 *)statebuf = pe_dmem_read(id, dmem_addr, 4);
	...
	dmem_addr += 4;
	len += sprintf(buf + len, "rx=%d ", cpu_to_be32(val));

	val = pe_dmem_read(id, dmem_addr, 4);
	if (do_clear && val)
		pe_dmem_write(id, 0, dmem_addr, 4);
	dmem_addr += 4;
	len += sprintf(buf + len, "tx=%d", cpu_to_be32(val));
	...
	dmem_addr += 4;
	if (val)
		len += sprintf(buf + len, " drop=%d", cpu_to_be32(val));
...
static ssize_t class_phy_stats(char *buf, int phy)
{
	...
	len += sprintf(buf + len, "  rx:   %10d, tx:   %10d, intf:  %10d, ipv4:    %10d, ipv6: %10d\n",
	...
	len += sprintf(buf + len, "  icmp: %10d, igmp: %10d, tcp:   %10d, udp:     %10d\n",
	...
	len += sprintf(buf + len, "  lp:   %10d, intf: %10d, l3:    %10d, chcksum: %10d, ttl:  %10d\n",
...
static ssize_t tmu_queue_stats(char *buf, int tmu, int queue)
{
	...
	len += sprintf(buf + len, "(teq) drop: %10d, tx: %10d (llm) head: %08x, tail: %08x, drop: %10d\n",
...
static ssize_t pfe_show_class(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	for (id = CLASS0_ID; id <= CLASS_MAX_ID; id++)
	{
		...
		len += sprintf(buf + len, "ldst=%04x ", val >> 16);
